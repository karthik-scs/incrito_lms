...
export function CoursesContent() {
  ...
  // Both Mentor and CM only see their assigned courses; only Admin sees all
  const isRestricted = user?.role === "Mentor" || user?.role === "Cohort Manager";
  ...
  async function loadAll() {
    ...
    if (isRestricted) requests.push(apiJson<{ course: { id: string } }[]>("/api/cohorts"));
  ...
  function openCreate() {
    setEditing(null); setTitle(""); setSlug(""); setSlugTouched(false);
    setDescription(""); setThumbnailUrl(null); setCategoryId(""); setMentorId("");
    setTagIds([]); setIsFree(true); setPrice(""); setUnlockMode("SEQUENTIAL");
    setFormError(null); setModalOpen(true);
  }

  function openEdit(course: Course) {
    setEditing(course); setTitle(course.title); setSlug(course.slug);
    setSlugTouched(true); setDescription(course.description ?? "");
    setThumbnailUrl(course.thumbnailUrl); setCategoryId(course.categoryId ?? "");
    setMentorId(course.mentorId); setTagIds(course.tags.map((t) => t.tag.id));
    setIsFree(course.isFree);
    setPrice(course.priceInSmallestUnit ? String(course.priceInSmallestUnit / 100) : "");
    setUnlockMode(course.unlockMode); setFormError(null); setModalOpen(true);
  }

  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    setFormError(null);
    if (!mentorId) { setFormError("Select a mentor for this course"); return; }
    setSubmitting(true);
    const payload = {
      title, slug, description: description || undefined,
      thumbnailUrl: thumbnailUrl || undefined, categoryId: categoryId || undefined,
      tagIds, mentorId, isFree,
      priceInSmallestUnit: isFree ? undefined : Math.round(Number(price) * 100),
      currency: "INR", unlockMode,
    };
  ...
  const visibleCourses = useMemo(() => {
    if (isRestricted && assignedCourseIds !== null) {
      return courses.filter((c) => assignedCourseIds.has(c.id));
    }
    return courses;
  }, [courses, isRestricted, assignedCourseIds]);
          ...
          <div key={course.id} className="bg-surface border border-border rounded-2xl overflow-hidden flex flex-col">
            ...
            <div className="p-5 flex flex-col gap-3 flex-1">
              ...
              <div className="flex flex-wrap gap-1">
                ...
                {course.tags.map((t) => (
                  <Badge key={t.tag.id} variant="accent">{t.tag.name}</Badge>
                ))}
              </div>
        ...
        <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "Edit Course" : "New Course"} maxWidth="max-w-2xl">
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium text-text-secondary" htmlFor="course-title">Title</label>
                <input
                  id="course-title"
                  value={title}
              ...
              <div>
                <label className="text-sm font-medium text-text-secondary" htmlFor="course-slug">Slug</label>
                <input
                  id="course-slug"
                  value={slug}
            ...
            <div>
              <label className="text-sm font-medium text-text-secondary" htmlFor="course-description">Description</label>
              <textarea
